...
//Nuevos shader para los colores
static const char* vShaderR = "shaders/shaderR.vert"; //Shader rojo
static const char* vShaderV = "shaders/shaderV.vert"; //Shader verde
static const char* vShaderA = "shaders/shaderA.vert"; //Shader azul
static const char* vShaderC = "shaders/shaderC.vert"; //Shader café
static const char* vShaderVO = "shaders/shaderVO.vert"; //Shader verde oscuro
...
void CreateShaders()
{
	...
	shader7->CreateFromFiles(vShaderVO, fShader);
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		*/
		...
		angulo += 0.01;
		//Inicializar matriz de dimensión 4x4 que servirá como matriz de modelo para almacenar las 
		...
		model = glm::translate(model, glm::vec3(0.25f, 0.0f, -2.0f));
		model = glm::scale(model, glm::vec3(0.4f, 0.4f, 0.4f));
		model = glm::rotate(model, glm::radians(angulo), glm::vec3(0.f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		meshList[1]->RenderMesh(); //Ya que es el cubo en el mesh list debe ser el indice 1
